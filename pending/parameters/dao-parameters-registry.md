...
| Majority Judgment: Grade Scale | Excellent / Very Good / Good / Acceptable / Poor | The five-grade scale for Majority Judgment election votes (§6.2.4). **The order is normative** — Excellent is the highest grade and Poor the lowest, and every median, ranking, and Minimum Median Grade comparison is a comparison of rank on this scale. For implementation the grades carry the fixed indices Excellent = 4, Very Good = 3, Good = 2, Acceptable = 1, Poor = 0; the indices are an encoding of the order, not scores to be averaged. A voter assigns exactly one grade to every candidate, and there is no Abstain grade |
...
| MJ Minimum Median Grade             | Good      | The lowest median grade at which a candidate may be seated in a Majority Judgment election at standard quorum (Proposal & Voting Framework §6.2.4; Elections & Role Governance Policy §7A.1). A candidate ranked within the open seats but below this floor is not seated, and the seat passes to the next-ranked electable candidate. This is the Majority Judgment counterpart to the Election Stage 2 Approval Threshold — a threshold on rank rather than on vote share |
| MJ Rerun Quorum                     | 7% — **the same as the first round** | Quorum for the single permitted rerun of a Majority Judgment election that closed below the Minimum Participation for Elections (Elections & Role Governance Policy §7A.3). Deliberately **not** reduced. The two-stage rerun can safely lower its quorum because it pairs the reduction with a raised approval threshold, and a share-of-votes threshold is a real constraint on a bloc. Majority Judgment has no share threshold to raise: a voter block holding more than half of the voting power cast sets the median grade and can name any grade it chooses, so a lower quorum would reduce the voting power needed to control the outcome with nothing offsetting it — and would make a rerun cheaper to control than the round it rescues, rewarding turnout suppression. The remedy for thin turnout here is more time, not a lower bar. A round that meets quorum but seats nobody is not a quorum failure and does not rerun |
| MJ Rerun Minimum Median Grade       | Good — **the same as the first round** | The electability floor is unchanged on rerun, for the reason given above: raising it would not constrain a voter block that sets the median, because setting the median means choosing the grade. It would burden a genuine broad electorate while leaving a concentrated one unaffected |
| MJ Rerun Voting Period              | Double the standard Election Voting Duration | Extended voting window for a Majority Judgment rerun. This is the entire remedy the rerun offers: the same electorate, measured against the same snapshot and the same thresholds, given twice as long to participate. Runs from the RAC's rerun notice (Elections & Role Governance Policy §7A.3) |
| Max MJ Reruns                       | 1         | A Majority Judgment election may be re-run at the reduced rerun quorum at most once; if the rerun is still short, no candidate is elected and all seats are referred to the vacancy / founding re-run process (Elections & Role Governance Policy §7A.3, §11, §17.1) |
