...
| MJ Rerun Quorum                     | 7% — **the same as the first round** | Quorum for the single permitted rerun of a Majority Judgment election that closed below the Minimum Participation for Elections (Elections & Role Governance Policy §7A.3). Deliberately **not** reduced: Majority Judgment has no share-of-votes threshold that could be raised to compensate, so a lower quorum would make the rerun cheaper to control than the round it rescues. The remedy for thin turnout here is more time, not a lower bar. Full reasoning: Proposal & Voting Framework §6.2.4. A round that meets quorum but seats nobody is not a quorum failure and does not rerun |
...
| Max MJ Reruns                       | 1         | A Majority Judgment election may be re-run **at most once**. The rerun runs at the **same** quorum and the **same** Minimum Median Grade as the round it follows — see the MJ Rerun Quorum and MJ Rerun Minimum Median Grade rows above — and the only remedy it offers is the doubled MJ Rerun Voting Period: **more time, not a lower bar**. If the rerun also closes below quorum, no candidate is elected and all seats are referred to the vacancy / founding re-run process (Elections & Role Governance Policy §7A.3, §11, §17.1). This cap applies **per election**; it does not limit the number of fresh elections the founding re-run process in §17.1 may require |
