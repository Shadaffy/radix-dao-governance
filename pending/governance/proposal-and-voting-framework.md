...
A tie is broken only so far as the outcome requires: a tie that does not straddle the seat boundary or the electable boundary needs no resolution. Where the majority gauge does not resolve a tie that does affect the outcome — that is, where the tied candidates share the same median grade, the same p, and the same q — the RAC adjudicates the tie and records its determination as part of the published result (§6.5). This adjudication is a defined step of this mechanism, not a discretionary power exercised outside it.
